...
const QuickViewModal = ({ product, onClose, addToCart, setCurrentPage, setSelectedProduct, isWishlisted, toggleWishlist }) => {
  ...
  return (
    <>
      {/* Backdrop — full screen, always clickable to close */}
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 pointer-events-none">
        <div
          className="relative bg-white rounded-2xl shadow-luxury-xl w-full max-w-lg overflow-hidden pointer-events-auto animate-fadeInUp"
          onClick={(e) => e.stopPropagation()}
          style={{ maxHeight: 'min(85vh, 700px)' }}
        >
          {/* Close button */}
          <button onClick={onClose}
            className="absolute top-3 right-3 z-20 w-9 h-9 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-500 hover:text-gray-800 shadow-sm transition-all">
            <X className="w-4 h-4" />
          </button>

          {/* Scrollable content */}
          <div className="overflow-y-auto" style={{ maxHeight: 'min(85vh, 700px)' }}>
              ...
              {images.length > 1 && (
                <>
                  <button onClick={(e) => { e.stopPropagation(); prevImage(); }}
                    className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-700 shadow-sm">
                    <ChevronLeft className="w-4 h-4" />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); nextImage(); }}
                    className="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/80 backdrop-blur-sm rounded-full flex items-center justify-center text-gray-700 shadow-sm">
                    <ChevronRight className="w-4 h-4" />
                  </button>
                  <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
                    {images.map((_, i) => (
                      <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${
                        i === currentImageIndex ? 'w-4 bg-[#e84393]' : 'w-1.5 bg-white/60'
                      }`} />
                    ))}
                  </div>
                </>
            ...
            <div className="p-5">
              {product.category && (
                <span className="text-[10px] text-[#e84393] font-semibold uppercase tracking-wider mb-1.5 block">{product.category}</span>
              )}

              <h2 className="text-xl font-serif text-gray-900 mb-2 leading-tight">{product.name}</h2>
              ...
              <div className="space-y-2.5">
                {isOutOfStock ? (
                  <button disabled className="w-full bg-gray-100 text-gray-400 py-3 rounded-xl font-semibold cursor-not-allowed text-sm">
                    Out of Stock
                  </button>
                ) : (
                  <button onClick={handleAddToCart}
                    className={`w-full py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 text-sm ${
                      addedToCart
                        ? 'bg-emerald-500 text-white'
                        : 'bg-gradient-to-r from-[#e84393] to-[#a855f7] text-white hover:from-[#d63384] hover:to-[#9333ea] shadow-md hover:shadow-lg'
                    }`}>
                    {addedToCart ? (
                      <><svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg> Added!</>
                    ) : hasOptions ? (
                      <>Select Options</>
                    ) : (
                ...
                <button onClick={handleViewFull}
                  className="w-full py-2.5 rounded-xl font-medium text-gray-500 hover:text-[#e84393] transition-colors flex items-center justify-center gap-1.5 text-sm">
